    ...
    def run(self, tmp=None, task_vars=None):
        ...
        try:
            ...
            if network:
                ...
                bridges = { k: v['bridge'] for k, v in network.items() if 'bridge' in v }
                if not bridges:
                    return result

                conf['autobridge_interfaces'] = ' '.join(bridges.keys())
                conf.update({ 'autobridge_{}'.format(k): ' '.join(v)
                    for k, v in bridges.items() if v })

            if not conf:
                return result
            module_args = dict(
                file=self._task.args.get('file', '/etc/rc.conf.d/bridge'),
                state='present',
                conf=conf,
            )
